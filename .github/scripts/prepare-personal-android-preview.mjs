...
if (!sourceRootArgument || !versionCodeArgument) {
  throw new Error("Usage: prepare-personal-android-preview.mjs <source-root> <version-code>");
}
...
const manifestPath = path.join(sourceRoot, "apps/mobile/android/app/src/main/AndroidManifest.xml");
const buildGradlePath = path.join(sourceRoot, "apps/mobile/android/app/build.gradle");
...
buildGradle = buildGradle.replace(versionCodeMatches[0][0], `        versionCode ${versionCode}`);
...
const releaseSigningIndex = buildGradle.indexOf(releaseSigning, releaseBlockIndex);
